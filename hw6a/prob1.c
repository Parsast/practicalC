...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
	...
	p_expr_token top = NULL;
	p_expr_token stack = NULL;
	struct token_queue *pqueue_postfix;
	pqueue_postfix = malloc(sizeof(struct token_queue));
	pqueue_postfix->back = NULL;
	pqueue_postfix->front = NULL;
	while(pqueue_infix->front){
		top = dequeue(pqueue_infix);
		switch (top->type)
		{
		case OPERAND:
			enqueue(pqueue_postfix,top);
			break;
		
		case OPERATOR:
			if (stack != NULL){
				while(
				(stack != NULL) && (stack->type == OPERATOR) &&
				( 
					(op_precedences[stack->value.op_code]>op_precedences[top->value.op_code])||
					(op_precedences[stack->value.op_code]==op_precedences[top->value.op_code]&&
					op_associativity[op_precedences[top->value.op_code]]== LEFT)
					)

				){
					enqueue(pqueue_postfix,pop(&stack));
				}
			}
			push(&stack,top);
			break;
		case LPARENS:
			...
			break;
		case RPARENS:
			while(stack!=NULL && stack->type != LPARENS){
				enqueue(pqueue_postfix,pop(&stack));
			}
			if(stack->type == LPARENS)
				pop(&stack);
			break;
		default:
			break;
	...
	/* TODO: process postfix-ordered queue and return final answer;
	   ...
	   while (pqueue_postfix->front)
	   {
		...
		case OPERATOR:
			...
			if(op_operands[ptoken->value.op_code]==2){

				p_expr_token operand1 = pop(&stack);
				p_expr_token operand2 = pop(&stack);
				union token_value value;
				p_expr_token result_token;
				float result;
				switch (ptoken->value.op_code)
				{
				case ADD:
					result = operand1->value.operand + operand2->value.operand;
					value.operand = result;
					result_token = new_token(OPERAND, value);
					push(&stack, result_token);
					break;
				case SUBTRACT:
					result = operand2->value.operand - operand1->value.operand;
					value.operand = result;
					result_token = new_token(OPERAND, value);
					push(&stack, result_token);
					break;
				...
				p_expr_token operand1 = pop(&stack);
				float result = -operand1->value.operand;
				union token_value value;
				value.operand = result;
				p_expr_token result_token = new_token(OPERAND, value);
				push(&stack, result_token);
				free(operand1);
			}
		default:
