...
{
  struct student* stdupra = (struct student*)a;
  struct student* stdprb = (struct student*)b;
  int result = strcmp(stdupra->fname, stdprb->fname);
...
int compare_last_name(const void* a,const void* b)
{
  ...
  struct student* stdupra = (struct student*)a;
  struct student* stdprb = (struct student*)b;
  int result = strcmp(stdupra->lname, stdprb->lname);
...
{
  struct student* st =  (struct student* ) prec;
  int threshhold = *((int *)arg);
  if (st->age > threshhold){
    printrec(st,threshhold);
  }
}
